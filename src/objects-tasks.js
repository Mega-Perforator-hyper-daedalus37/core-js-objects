...
function shallowCopy(obj) {
  const anotherObject = {};
  const target = Object.assign(anotherObject, obj);
  return target;
}
...
function mergeObjects(objects) {
  return objects.reduce((mergedObj, obj) => {
    const dushnilovo = mergedObj;
    Object.entries(obj).forEach(([key, value]) => {
      dushnilovo[key] = (mergedObj[key] ?? 0) + value;
    });
    return dushnilovo;
  }, {});
}
...
function isEmptyObject(obj) {
  return JSON.stringify(Object.keys(obj)) === JSON.stringify([]);
}
...
function sellTickets(queue) {
  const number = queue.reduce((acc, cusVal) => {
    if (acc === 0) {
      return 0 + cusVal;
    }
    return acc >= cusVal ? acc + cusVal : false;
  }, 0);
